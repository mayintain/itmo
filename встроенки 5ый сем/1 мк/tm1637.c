...
void tm1637_display_number(int number) {
  ...
  // массив хранит коды четырех разрядов дисплея
  uint8_t digits[4] = {0x00, 0x00, 0x00, 0x00};

  // 0 - число положительное или равно нулю
  ...
  if (number < 0) {
    ...
    // убираем минус, чтобы дальше разбить число на отдельные цифры
    number = -number;
  }

  // выводим единицы в крайний правый разряд
  digits[3] = digit_codes[number % 10];

  // если есть десятки, выводим их
  if (number >= 10) {
    digits[2] = digit_codes[(number / 10) % 10];
  }

  // если есть сотни, выводим их
  if (number >= 100) {
    digits[1] = digit_codes[(number / 100) % 10];
  }

  // если есть тысячи, выводим их
  if (number >= 1000) {
  ...
  if (isNegative) {
    ...
    // для однозначного числа минус ставим перед единицами
    if (number < 10) {
      digits[2] = 0x40;
    }

    // для двузначного числа минус ставим перед десятками
    else if (number < 100) {
      digits[1] = 0x40;
    }

    // для трехзначного числа минус ставим в первый разряд
    else if (number < 1000) {
      digits[0] = 0x40;
    }
  }
  ...
  // передаем данные для всех четырех разрядов
  for (int i = 0; i < 4; i++) {
...
// отображение данных в одном разряде дисплея
void tm1637_display_digit(uint8_t digit, uint8_t data) {
  // режим записи в фиксированный адрес
  tm1637_start();
  tm1637_write_byte(0x44);
  tm1637_stop();

  // устанавливаем адрес нужного разряда и передаем данные
  tm1637_start();
  tm1637_write_byte(0xC0 + digit);
