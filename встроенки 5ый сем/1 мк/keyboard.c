...
void scanKeyboard() {
  // Сканируем клавиатуру каждые 100 мс.
  if (tickCount - lastScanTime > 100) {
    lastScanTime = tickCount;
    char currentKey = readKey(); // какая клавиша сейчас нажата

    if (currentKey != '\0' && currentKey != lastKey) { // проверка (удержание ли это предыдущей клавиши)

      printf("Pressed: %c\n", currentKey); // запоминаем клавишу, чтобы одно нажатие не ощущалось, как удержание
      lastKey = currentKey;
      processKey(currentKey);

    } else if (currentKey == '\0') { // если ни одна клавиша не нажата, то сбрасываем
      lastKey = '\0';
    }
  }
}